...
const C = {
  ...
  text: "#e8ecf4", muted: "#7a8099", dim: "#4a5068",
};
...
const Modal = ({ title, onClose, children }) => (
  <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.75)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
    ...
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 18, width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto", padding: 28 }}>
    ...
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 400, padding: 20 }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          <div style={{ background: C.accent, borderRadius: 16, padding: 14, display: "inline-flex", marginBottom: 16 }}>
            <Icon d={icons.cube} size={28} color="#fff" />
          </div>
        ...
        <Card>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <Btn onClick={handle} loading={loading} full size="lg">
    ...
    <div style={{ padding: "24px 20px 18px", borderBottom: `1px solid ${C.border}` }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ background: C.accent, borderRadius: 10, padding: 8, display: "flex" }}>
          <Icon d={icons.cube} size={18} color="#fff" />
        </div>
        <div>
          <div style={{ fontWeight: 800, fontSize: 14, color: C.text }}>Print3D</div>
          <div style={{ fontSize: 11, color: C.muted }}>Manager</div>
        </div>
      </div>
    </div>
    <nav style={{ flex: 1, padding: "12px 10px", overflowY: "auto" }}>
      {NAV.map(item => (
        <button key={item.id} onClick={() => setActive(item.id)}
          ...
          <Icon d={icons[item.icon]} size={17} color={active === item.id ? C.accent : C.muted} />
          {item.label}
        </button>
      ))}
    </nav>
...
// ── DASHBOARD ──────────────────────────────────────────────────────────────
const Dashboard = ({ filamentos, pedidos, clientes }) => {
  ...
  const filLivre = filamentos.reduce((s, f) => s + (f.peso_total - (f.consumido || 0)), 0);
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <Card>
          <h3 style={{ margin: "0 0 16px", fontSize: 15, fontWeight: 700, color: C.text }}>Estoque de Filamentos</h3>
          {filamentos.length === 0 ? <p style={{ color: C.dim, fontSize: 13 }}>Nenhum filamento cadastrado</p>
            : filamentos.map(f => {
              const livre = f.peso_total - (f.consumido || 0);
              const pct = Math.max(0, Math.min(100, (livre / f.peso_total) * 100));
              return (
                <div key={f.id} style={{ marginBottom: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 4 }}>
                    <span style={{ fontWeight: 600, color: C.text }}>{f.marca} {f.material}</span>
                    <span style={{ color: C.muted }}>{livre.toFixed(0)}g / {f.peso_total}g</span>
                  </div>
                  <div style={{ height: 6, background: C.surface, borderRadius: 3 }}>
                    <div style={{ height: "100%", width: `${pct}%`, background: pct > 50 ? C.green : pct > 20 ? C.yellow : C.red, borderRadius: 3 }} />
                  </div>
                </div>
              );
            })}
...
const Orcamento = ({ filamentos, clientes, configs, onAddPedido }) => {
  ...
  const calcular = () => {
    ...
    const custoFil = fil ? (parseFloat(consumoG) || 0) * (fil.valor_pago / fil.peso_total) : 0;
    const mins = (parseFloat(tempH) || 0) * 60 + (parseFloat(tempM) || 0);
    const custoHora = configs?.custo_hora || 15;
    const custoPrint = (mins / 60) * custoHora;
    const custoEnergia = (mins / 60) * 0.2 * (configs?.energia_kwh || 0.85);
    const custoMao = (parseFloat(tempoMao) || 0) / 60 * custoHora;
    ...
    const addUrg = custoTotal * ((parseFloat(urgencia) || 0) / 100);
    const addMkt = custoTotal * ((configs?.marketplace || 0) / 100);
    const addCart = custoTotal * ((configs?.cartao || 0) / 100);
    const addNF = custoTotal * ((configs?.nf || 0) / 100);
    const base = custoTotal + addUrg + addMkt + addCart + addNF;
    const preco = base / (1 - margem / 100);
    const lucro = preco - base;
    setResultado({ custoFil, custoPrint, custoEnergia, custoMao, custoMod, custoProducao, custoTotal, preco, lucro });
  };

  const salvar = async () => {
    if (!resultado || !nome) return alert("Preencha o nome do produto e calcule primeiro.");
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 20 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <Card>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <Select label="Filamento" value={filId} onChange={setFilId}
                options={[{ value: "", label: "— Selecionar —" }, ...filamentos.map(f => ({ value: f.id, label: `${f.marca} ${f.material} (${f.cor})` }))]} />
            ...
            {filId && (() => {
              ...
              if (!f) return null;
              return <div style={{ marginTop: 10, padding: 10, background: C.surface, borderRadius: 8, fontSize: 12, color: C.muted }}>
                Custo/g: {brl(f.valor_pago / f.peso_total)} · Disponível: {(f.peso_total - (f.consumido || 0)).toFixed(0)}g · Custo estimado: {brl((parseFloat(consumoG) || 0) * f.valor_pago / f.peso_total)}
              </div>;
            })()}
          </Card>
          <Card>
        ...
        <div>
          <Card style={{ position: "sticky", top: 20 }}>
            ...
            {resultado ? (
              <>
                {[["Filamento", resultado.custoFil], ["Máquina/impressão", resultado.custoPrint], ["Energia", resultado.custoEnergia], ["Mão de obra", resultado.custoMao], ["Modelagem", resultado.custoMod]].map(([l, v]) => (
                ...
                <Btn onClick={salvar} icon="plus" full style={{ marginTop: 14 }}>Criar Pedido</Btn>
              </>
            ) : (
...
// ── FILAMENTOS ─────────────────────────────────────────────────────────────
const Filamentos = ({ filamentos, onAdd, onDel }) => {
  const [modal, setModal] = useState(false);
  const [form, setForm] = useState({ marca: "", material: "PLA", cor: "", peso_total: 1000, valor_pago: "" });
  const [loading, setLoading] = useState(false);

  const add = async () => {
    if (!form.marca || !form.valor_pago) return alert("Preencha marca e valor.");
    setLoading(true);
    await onAdd({ ...form, peso_total: parseFloat(form.peso_total), valor_pago: parseFloat(form.valor_pago), consumido: 0 });
    setModal(false); setForm({ marca: "", material: "PLA", cor: "", peso_total: 1000, valor_pago: "" }); setLoading(false);
  };
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div><h1 style={{ fontSize: 24, fontWeight: 800, color: C.text, margin: 0 }}>Filamentos</h1>
          <p style={{ color: C.muted, fontSize: 14, margin: "4px 0 0" }}>Gerencie seu estoque</p></div>
        <Btn onClick={() => setModal(true)} icon="plus">Novo Filamento</Btn>
      </div>
      {filamentos.length === 0 ? (
        <Card style={{ textAlign: "center", padding: 60 }}>
          <Icon d={icons.cube} size={48} color={C.dim} />
          <p style={{ color: C.dim, marginTop: 12 }}>Nenhum filamento cadastrado</p>
        </Card>
      ) : (
              ...
              <Card key={f.id}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
                  <div><div style={{ fontWeight: 700, color: C.text, fontSize: 15 }}>{f.marca}</div>
                    ...
                    {f.cor && <span style={{ fontSize: 12, color: C.muted, marginLeft: 6 }}>{f.cor}</span>}</div>
                  <button onClick={() => onDel(f.id)} style={{ background: "none", border: "none", color: C.dim, cursor: "pointer" }}><Icon d={icons.trash} size={16} /></button>
                </div>
                <div style={{ marginBottom: 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: C.muted, marginBottom: 6 }}>
                    <span>Disponível: <strong style={{ color: pct > 30 ? C.green : C.red }}>{livre.toFixed(0)}g</strong></span>
                    <span>Total: {f.peso_total}g</span>
                  </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 12 }}>
                  <div style={{ background: C.surface, borderRadius: 8, padding: 10 }}>
                    <div style={{ color: C.muted }}>Valor pago</div>
                    <div style={{ fontWeight: 700, color: C.text }}>{brl(f.valor_pago)}</div>
                  </div>
                  <div style={{ background: C.surface, borderRadius: 8, padding: 10 }}>
                    <div style={{ color: C.muted }}>Custo/g</div>
                    <div style={{ fontWeight: 700, color: C.accent }}>{brl(f.valor_pago / f.peso_total)}</div>
                  </div>
                </div>
              </Card>
            );
      ...
      {modal && (
        <Modal title="Novo Filamento" onClose={() => setModal(false)}>
          <div style={{ display: "grid", gap: 14 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <Input label="Marca" value={form.marca} onChange={v => setForm({ ...form, marca: v })} placeholder="Ex: Esun" />
              <Select label="Material" value={form.material} onChange={v => setForm({ ...form, material: v })}
                options={MATERIAIS.map(m => ({ value: m, label: m }))} />
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <Input label="Cor" value={form.cor} onChange={v => setForm({ ...form, cor: v })} placeholder="Ex: Branco" />
              <Input label="Peso total" value={form.peso_total} onChange={v => setForm({ ...form, peso_total: v })} type="number" suffix="g" />
            </div>
            <Input label="Valor pago" value={form.valor_pago} onChange={v => setForm({ ...form, valor_pago: v })} type="number" prefix="R$" />
            <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
              <Btn onClick={add} loading={loading} full>Adicionar</Btn>
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div><h1 style={{ fontSize: 24, fontWeight: 800, color: C.text, margin: 0 }}>Pedidos</h1>
          <p style={{ color: C.muted, fontSize: 14, margin: "4px 0 0" }}>Gerencie todos os seus pedidos</p></div>
        <Btn onClick={() => setModal(true)} icon="plus">Novo Pedido</Btn>
      </div>
      <div style={{ marginBottom: 16 }}>
        <Input value={busca} onChange={setBusca} placeholder="🔍  Buscar pedido..." />
      </div>
      {filtrados.length === 0 ? (
        <Card style={{ textAlign: "center", padding: 60 }}>
          <Icon d={icons.printer} size={48} color={C.dim} /><p style={{ color: C.dim, marginTop: 12 }}>Nenhum pedido encontrado</p>
        </Card>
      ) : (
        <Card style={{ padding: 0, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr style={{ background: C.surface }}>
              {["Produto", "Cliente", "Data", "Preço", "Status", ""].map(h => (
            ...
            <tbody>
              {filtrados.map((p, i) => (
                <tr key={p.id} style={{ borderTop: `1px solid ${C.border}`, background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,.01)" }}>
                  ...
                  <td style={{ padding: "14px 16px" }}>
                    <button onClick={() => onDel(p.id)} style={{ background: "none", border: "none", color: C.dim, cursor: "pointer" }}><Icon d={icons.trash} size={15} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {modal && (
        <Modal title="Novo Pedido" onClose={() => setModal(false)}>
          <div style={{ display: "grid", gap: 14 }}>
            <Input label="Nome do produto" value={form.nome} onChange={v => setForm({ ...form, nome: v })} />
            <Select label="Cliente" value={form.cliente_id} onChange={v => setForm({ ...form, cliente_id: v })}
              options={[{ value: "", label: "— Sem cliente —" }, ...clientes.map(c => ({ value: c.id, label: c.nome }))]} />
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <Input label="Preço de venda" value={form.preco_final} onChange={v => setForm({ ...form, preco_final: v })} type="number" prefix="R$" />
              <Select label="Status" value={form.status} onChange={v => setForm({ ...form, status: v })} options={STATUS.map(s => ({ value: s, label: s }))} />
            </div>
...
const Clientes = ({ clientes, pedidos, onAdd, onDel }) => {
  ...
  const add = async () => {
    if (!form.nome) return;
    await onAdd(form);
    setModal(false); setForm({ nome: "", email: "", tel: "", cidade: "" });
  };
  ...
  return (
    <div>
      ...
      {filtrados.length === 0 ? (
        <Card style={{ textAlign: "center", padding: 60 }}>
          <Icon d={icons.users} size={48} color={C.dim} /><p style={{ color: C.dim, marginTop: 12 }}>Nenhum cliente cadastrado</p>
        </Card>
      ) : (
            ...
            return (
              <Card key={c.id}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                  ...
                  <button onClick={() => onDel(c.id)} style={{ background: "none", border: "none", color: C.dim, cursor: "pointer" }}><Icon d={icons.trash} size={15} /></button>
                </div>
      ...
      {modal && (
        <Modal title="Novo Cliente" onClose={() => setModal(false)}>
          <div style={{ display: "grid", gap: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <Input label="Telefone" value={form.tel} onChange={v => setForm({ ...form, tel: v })} />
              <Input label="Cidade" value={form.cidade} onChange={v => setForm({ ...form, cidade: v })} />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <Btn onClick={add} full>Cadastrar</Btn>
              <Btn onClick={() => setModal(false)} variant="ghost" full>Cancelar</Btn>
            </div>
          </div>
        </Modal>
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 24 }}>
        ...
        <StatCard label="Lucro Líquido" value={brl(lucro)} sub={`margem ${margem}%`} color={C.accent} icon="chart" />
      </div>
      <Card>
        <h3 style={{ margin: "0 0 18px", fontSize: 15, fontWeight: 700, color: C.text }}>Pedidos Finalizados</h3>
        {pagos.length === 0 ? <p style={{ color: C.dim, fontSize: 13 }}>Nenhum pedido finalizado ainda</p>
          : <table style={{ width: "100%", borderCollapse: "collapse" }}>
    ...
    <div>
      ...
      <div style={{ display: "grid", gap: 16, maxWidth: 600 }}>
        <Card>
          ...
          <div style={{ display: "grid", gap: 14 }}>
            <Input label="Custo por hora (máquina + mão de obra)" value={form.custo_hora} onChange={v => setForm({ ...form, custo_hora: parseFloat(v) || 0 })} type="number" prefix="R$" suffix="/h" help="Valor cobrado por hora de impressão" />
            <Input label="Energia elétrica" value={form.energia_kwh} onChange={v => setForm({ ...form, energia_kwh: parseFloat(v) || 0 })} type="number" prefix="R$" suffix="/kWh" help="Tarifa de energia da sua cidade" />
          </div>
        </Card>
        <Card>
          <h3 style={{ margin: "0 0 18px", fontSize: 14, fontWeight: 700, color: C.accent }}>📊 Margens e Taxas</h3>
          <div style={{ display: "grid", gap: 14 }}>
            <Input label="Margem de lucro padrão" value={form.margem_lucro} onChange={v => setForm({ ...form, margem_lucro: parseFloat(v) || 0 })} type="number" suffix="%" />
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <Input label="Marketplace (%)" value={form.marketplace} onChange={v => setForm({ ...form, marketplace: parseFloat(v) || 0 })} type="number" suffix="%" help="Mercado Livre, Shopee..." />
              <Input label="Cartão (%)" value={form.cartao} onChange={v => setForm({ ...form, cartao: parseFloat(v) || 0 })} type="number" suffix="%" />
            </div>
            <Input label="Nota Fiscal (%)" value={form.nf} onChange={v => setForm({ ...form, nf: parseFloat(v) || 0 })} type="number" suffix="%" />
          </div>
        </Card>
...
export default function App() {
  ...
  const addFilamento = async (data) => {
    const { data: r } = await supabase.from("filamentos").insert({ ...data, user_id: user.id }).select().single();
    if (r) setFilamentos(prev => [r, ...prev]);
  };
  const delFilamento = async (id) => {
    await supabase.from("filamentos").delete().eq("id", id);
    setFilamentos(prev => prev.filter(f => f.id !== id));
  };
  const addCliente = async (data) => {
    const { data: r } = await supabase.from("clientes").insert({ ...data, user_id: user.id }).select().single();
    if (r) setClientes(prev => [r, ...prev]);
  };
  ...
  const addPedido = async (data) => {
    const { data: r } = await supabase.from("pedidos").insert({ ...data, user_id: user.id }).select().single();
    if (r) setPedidos(prev => [r, ...prev]);
  };
  const updPedido = async (id, data) => {
    await supabase.from("pedidos").update(data).eq("id", id);
  ...
  const saveConfigs = async (data) => {
    ...
    const { data: r } = await supabase.from("configs").upsert(payload, { onConflict: "user_id" }).select().single();
    if (r) setConfigs(r);
  };
  ...
  const pages = {
    dashboard: <Dashboard filamentos={filamentos} pedidos={pedidos} clientes={clientes} />,
    orcamento: <Orcamento filamentos={filamentos} clientes={clientes} configs={configs} onAddPedido={addPedido} />,
    filamentos: <Filamentos filamentos={filamentos} onAdd={addFilamento} onDel={delFilamento} />,
    pedidos: <Pedidos pedidos={pedidos} clientes={clientes} onAdd={addPedido} onUpd={updPedido} onDel={delPedido} />,
    clientes: <Clientes clientes={clientes} pedidos={pedidos} onAdd={addCliente} onDel={delCliente} />,
    financeiro: <Financeiro pedidos={pedidos} />,
    configs: <Configs configs={configs} onSave={saveConfigs} />,
  };

  return (
    <div style={{ fontFamily: "'DM Sans', 'Segoe UI', sans-serif", background: C.bg, color: C.text, minHeight: "100vh", display: "flex" }}>
      <style>{`* { box-sizing: border-box; margin: 0; padding: 0; } input, select { color-scheme: dark; } ::-webkit-scrollbar { width: 6px; } ::-webkit-scrollbar-track { background: ${C.surface}; } ::-webkit-scrollbar-thumb { background: ${C.border}; border-radius: 3px; } @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700;800;900&display=swap');`}</style>
